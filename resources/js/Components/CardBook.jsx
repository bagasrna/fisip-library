export default function CardBook({title, author, rate, publisher, image}) {
    return (
        <div className="card p-5 max-w-xs rounded-3xl" style={{ boxShadow: "0px 0px 10px rgba(0, 0, 0, 0.25)" }}>
            ...
            <div>
                <h3 className="text-lg my-2 font-bold">{window.location.pathname === "/result" ? title : "Kitab Kawin"}</h3>
                <p className="my-1">{window.location.pathname === "/result" ? author : "nama penulis"}</p>
            </div>
            ...
            <div>
                <p>{window.location.pathname === "/result" ? publisher : "contoh penerbit"}</p>
            </div>
        </div>
    );
}