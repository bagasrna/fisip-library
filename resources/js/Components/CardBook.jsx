export default function CardBook({ title, author, rate, publisher, image, category }) {
    return (
        <div className="relative card p-5 max-w-xs rounded-3xl min-h-[200px] shadow-none hover:shadow-lg transition duration-300 border-2">
            {/* <div className="card-image min-h-[450px]">
                <img src={window.location.pathname === "/result" ? image : "/images/book-cover.svg"} width="100%" />
            </div> */}
            <div>
                <h3 className="text-lg my-2 font-bold">{window.location.pathname === "/result" ? title : title}</h3>
            </div>
            <div className="absolute bottom-0 mb-5 mt-2">
                <p className="my-1">{window.location.pathname === "/result" ? author : author}</p>
                {/* <div className="card-rating flex items-center my-2">
                ...
                <svg className="w-5 h-5 text-yellow-300" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                <svg className="w-5 h-5 text-yellow-300" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path></svg>
                <span className="bg-blue-100 text-blue-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded dark:bg-blue-200 dark:text-blue-800 ml-3">{window.location.pathname === "/result" ? rate : "5.0"}</span>
            </div> */}
                <div>
