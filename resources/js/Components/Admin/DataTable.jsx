export default function DataTable({ books, setOpenDialog, setBook, setRole, setOpenDeleteDialog }) {
    return (
                            ...
                            return (
                                <tr key={item?.id} className="w-full flex">
                                    ...
                                    <td className={`border-t-2 //border-l-2 //border-r-2 ${item?.id === books?.data?.length ? "border-b-2" : ""} w-1/4 p-2 flex justify-center`}>
                                        ...
                                        <button onClick={() => setOpenDeleteDialog(true)} className="trash-logo ml-5 text-red-400 hover:text-red-300 transition duration-150">
                                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
